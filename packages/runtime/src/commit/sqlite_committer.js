 ...
 */
export class SQLiteCommitter {
    constructor(dbPath) {
        this.id = 'sqlite-v0';
        this.version = '0.1.0';
        this.description = 'Event-sourced append-only SQLite committer with artifact support';
        this.db = null;
        this.dbPath = dbPath;
    }
    /**
    ...
    async commit(req) {
        ...
        const existing = (await this.getAsync('SELECT commit_id FROM commits WHERE commit_key = ?', [req.commitKey]));
        if (existing) {
        ...
        try {
            ...
            await this.runAsync(`INSERT INTO commits
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            ...
            for (const event of req.events) {
                await this.runAsync(`INSERT OR IGNORE INTO events
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            await this.runAsync(`INSERT OR REPLACE INTO source_state
         ...
         VALUES (?, ?, ?, ?, ?)`, [
            ...
            if (req.artifacts) {
                for (const [name, ref] of Object.entries(req.artifacts)) {
                    await this.runAsync(`INSERT INTO artifacts
        ...
        for (const mutation of mutations) {
            if (mutation.operation === 'insert') {
                if (mutation.view === 'curated_docs') {
                    await this.runAsync(`INSERT OR IGNORE INTO curated_docs
             (doc_id, source_item_id, payload, last_event_id)
             VALUES (?, ?, ?, ?)`, [
                        mutation.key,
                        mutation.data?.source_item_id,
                        canonicalJson(mutation.data?.payload),
                        mutation.data?.last_event_id
                    ]);
                }
                else if (mutation.view === 'hold_index') {
                    await this.runAsync(`INSERT OR IGNORE INTO hold_index
             (hold_id, source_item_id, event_id, reason)
             VALUES (?, ?, ?, ?)`, [
                        mutation.key,
                        mutation.data?.source_item_id,
                        mutation.data?.event_id,
    ...
    async rebuildToHead() {
      ...
      `);
            ...
            const events = (await this.allAsync('SELECT * FROM events ORDER BY rowid'));
            for (const event of events) {
                // Replay item_decided events with decision === 'append'
                if (event.event_type === 'item_decided') {
                    const payload = JSON.parse(event.payload);
                    if (payload.decision === 'append') {
                        await this.runAsync(`INSERT OR REPLACE INTO curated_docs
               (doc_id, source_item_id, payload, last_event_id)
               VALUES (?, ?, ?, ?)`, [event.source_item_id, event.source_item_id, event.payload, event.event_id]);
                    }
                }
            }
            await this.execAsync('COMMIT TRANSACTION');
...
//# sourceMappingURL=sqlite_committer.js.map