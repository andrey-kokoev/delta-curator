 ...
 */
function generateCommitId(commitKey) {
    return createHash('sha256').update(commitKey).digest('hex');
}
/**
 * D1Committer: atomic, idempotent persistence via Cloudflare D1
 */
export class D1Committer {
    constructor(db) {
        this.db = db;
        this.id = 'd1-v0';
        this.version = '0.1.0';
        this.description = 'Event-sourced append-only D1 committer with artifact support';
    }
    /**
    ...
    async commit(req) {
        ...
        batch.push(this.db.prepare(`INSERT INTO commits
         (commit_id, commit_key, trace_id, source_id, old_state, new_state, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(commitId, req.commitKey, req.traceId, req.sourceId, canonicalJson(req.oldState), canonicalJson(req.newState), new Date().toISOString()));
        // Step 5: Insert events (idempotent by event_id PRIMARY KEY)
        for (const event of req.events) {
            batch.push(this.db.prepare(`INSERT OR IGNORE INTO events
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(event.event_id, commitId, event.event_type, event.event_version, event.payload_hash, event.trace_id, event.source_id, event.source_item_id, event.candidate_seq, event.observed_at, canonicalJson(event.payload)));
        }
        ...
        batch.push(this.db.prepare(`INSERT OR REPLACE INTO source_state
         (source_id, state, last_commit_id, last_commit_key, updated_at)
         VALUES (?, ?, ?, ?, ?)`).bind(req.sourceId, canonicalJson(req.newState), commitId, req.commitKey, new Date().toISOString()));
        // Step 8: Record artifacts (if any)
        if (req.artifacts) {
            for (const [name, artifact] of Object.entries(req.artifacts)) {
                batch.push(this.db.prepare(`INSERT INTO artifacts
             (artifact_key, commit_id, name, sha256, created_at)
             VALUES (?, ?, ?, ?, ?)`).bind(artifact.key, commitId, name, artifact.sha256, new Date().toISOString()));
            }
        }
     ...
     */
    getMutationStatements(mutation) {
        if (mutation.operation === 'insert') {
            if (mutation.view === 'curated_docs') {
                const data = mutation.data;
                return [
                    this.db.prepare(`INSERT OR REPLACE INTO curated_docs
             (doc_id, source_item_id, payload, last_event_id)
             VALUES (?, ?, ?, ?)`).bind(mutation.key, data?.source_item_id, canonicalJson(data?.payload), data?.last_event_id)
                ];
            }
            else if (mutation.view === 'hold_index') {
                const data = mutation.data;
                return [
                    this.db.prepare(`INSERT OR IGNORE INTO hold_index
             (hold_id, source_item_id, event_id, reason)
             VALUES (?, ?, ?, ?)`).bind(mutation.key, data?.source_item_id, data?.event_id, data?.reason)
                ];
            }
            else if (mutation.view === 'fingerprint_index') {
                const data = mutation.data;
                return [
                    this.db.prepare(`INSERT OR IGNORE INTO fingerprint_index
             (fingerprint, source_item_id, payload_hash, first_event_id)
             VALUES (?, ?, ?, ?)`).bind(mutation.key, data?.source_item_id, data?.payload_hash, data?.first_event_id)
                ];
            }
            else if (mutation.view === 'facet_index') {
                const data = mutation.data;
                return [
                    this.db.prepare(`INSERT OR IGNORE INTO facet_index
             (facet_id, event_id, facet_key, facet_value)
             VALUES (?, ?, ?, ?)`).bind(mutation.key, data?.event_id, data?.facet_key, data?.facet_value)
                ];
            }
        }
        return [];
            ...
            for (const event of events.results) {
                ...
                if (event.event_type === 'item_decided') {
                    ...
                    if (payload.decision === 'append') {
                        replayStatements.push(this.db.prepare(`INSERT OR REPLACE INTO curated_docs
                 (doc_id, source_item_id, payload, last_event_id)
                 VALUES (?, ?, ?, ?)`).bind(event.source_item_id, event.source_item_id, event.payload, event.event_id));
                    }
                }
            }
...
//# sourceMappingURL=d1_committer.js.map